...
class GameRunner {
    ...
    public static void main(String[] args) throws CloneNotSupportedException, InterruptedException{
        Ability[] tempAbilityList = {new Spear(), new Spear()};
        PlayerStatsTracker stats = new PlayerStatsTracker(15, 15, 3, 3, tempAbilityList);
        ...
        //playingField = ((Enemy) playingField.getSpace(6, 1).getEntity()).setUpAttack(playingField).get(3);
        playingField.addEntityToSpace(new Swordsman(), 6, 5);
        //playingField = ((Enemy) playingField.getSpace(6, 5).getEntity()).setUpAttack(playingField).get(3);
        playingField.addEntityToSpace(new Swordsman(), 8, 3);
        //playingField = ((Enemy) playingField.getSpace(8, 3).getEntity()).setUpAttack(playingField).get(3);
        ...
        while (playingField.enemiesRemain() && playingField.getPlayerSpace().getEntity().getHealth() > 0) { //this while loop repeats until the board is cleared of enemies
            ...
            while (true) { //this while loop repeats until the player's turn is over
                ...
                if (input.equals("1")) {
                    if (actionPoints > 0) {
                        clearAndPrintUI(playingField, stats);
                        System.out.println(stats.getAbilities()[0].howToUse());
                        ...
                        while (true) {
                            ...
                            if (input.toUpperCase().equals("X")) {
                                ...
                                try {
                                    ArrayList<Board> animations = stats.getAbilities()[0].doThing(input, playingField, stats);
                                    actionPoints--;
                                    ...
                                    clearAndPrintUI(playingField, stats);
                                    System.out.println(stats.getAbilities()[0].wrongUseMessage());
                    ...
                    clearAndPrintUI(playingField, stats);
                    System.out.println("this doesn't work yet. stay mad. ");
